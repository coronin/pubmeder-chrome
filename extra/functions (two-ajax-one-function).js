...
function simple_efetch(pmid) {
  $(document).ready(function() {
    args = {'apikey' : 'ab25c21c079653919d9b53213ac8cc6e',
            ...
            'id' : pmid};
    $.getJSON('http://entrezajax2.appspot.com/efetch?callback=?', args, function(d){
      var abstract = d.result[0].MedlineCitation.Article.Abstract.AbstractText;
      if (!abstract) abstract = 'NA';
      var title = d.result[0].MedlineCitation.Article.ArticleTitle;
      if (!title) title = 'NA';
      var journal = d.result[0].MedlineCitation.Article.Journal.ISOAbbreviation;
      if (!journal) journal = 'NA';
      var volume = d.result[0].MedlineCitation.Article.Journal.JournalIssue.Volume;
      if (!volume) volume = 'NA';
      var issue = d.result[0].MedlineCitation.Article.Journal.JournalIssue.Issue;
      if (!issue) volume = 'NA';
      var year = d.result[0].MedlineCitation.Article.Journal.JournalIssue.PubDate.Year;
      if (!year) year = 'NA';
      var page = d.result[0].MedlineCitation.Article.Pagination.MedlinePgn;
      if (!page) page = 'NA';
      var authors = 'NA';
      if (d.result[0].MedlineCitation.Article.AuthorList) {
        for(var j = 0; j < d.result[0].MedlineCitation.Article.AuthorList.length; j ++) {     
          if ( j == 0 ) {
            authors = d.result[0].MedlineCitation.Article.AuthorList[j].LastName + ' ' + d.result[0].MedlineCitation.Article. AuthorList[j].Initials;
          } else {
            authors += ', ' + d.result[0].MedlineCitation.Article.AuthorList[j].LastName + ' ' + d.result[0].MedlineCitation.Article. AuthorList[j].Initials;
          }
        }
      ...
      // return pmid_json
      $("<div/>").html(pmid_json).appendTo("#json");
    });
  });
}


function onePMIDjson(search_term) {
  ...
  $(document).ready(function(){
    $.ajax({
      ...
      success: function(xml) {
        ...
        if (pmid.length == 1) {
          simple_efetch(pmid.text());
        } else {
          // need return error more than 1
          alert(search_term + ' has none or multiple hits');
        }
      }
    });
  });
...
function allJSON() {
  ...
  var pmcCheck = /PMC/;
  var TimePattern = /10000000\d{14}/;
  var html = '<div id="json"><b>below is the json, delete last comma and add ]} to the end to complete it</b><br/><br/>{"pubmeder":[</div>';
  $("body").html(html);
  for (var i = (localStorage.length - 1); i >= 0 ; i--) {
    var tempKey = localStorage.key(i);
    var checkedTime = localStorage.getItem(tempKey);
